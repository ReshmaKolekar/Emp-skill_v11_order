...
import com.profile.model.Certification;
import com.profile.model.CertificationList;
...
public class EmployeeServiceImpl implements EmployeeService {
	 ...
	 */
	@Override
	public Object save(EmployeeDto employee) {
...
//		employee.setPassword(encodePass(emp.getPassword()));
	...
	public String sendMail(String to, boolean response) {
		...
		String URI = "http://EMAIL/Mail/sendmail";
	 ...
	 */
	@Override
	public EmployeeDto addEmpskill(int empId, SkillList skillList) throws RecordNotFoundException {
		...
		if (employee != null) {
			...
			throw new RecordNotFoundException("Record not found with employee _id : " + empId);
	 ...
	 */
	@Override
	public EmployeeDto getEmpById(int empId) throws RecordNotFoundException {
	...
	@Override
	public EmployeeDto updateEmpSkill(int empId, String skillName, Skills skill) throws RecordNotFoundException {

		EmployeeDto employee = employeeReo.findById(empId);
		if (employee == null)
			throw new RecordNotFoundException("Record not found with employee_id " + empId + " to update");
	...
	@Override
	public EmployeeDto updateEmpSkillSet(int empId, SkillList skillList) throws RecordNotFoundException {
		EmployeeDto employee = employeeReo.findById(empId);
		if (employee == null)
			throw new RecordNotFoundException("Record not found with employee_id " + empId + " to update");
		logger.info(" employee record to update : " + employee.toString());
		...
		logger.info(" Updated Employee-skill mapping :" + employeeReo.findById(empId).toString());
		return employeeReo.findById(empId);
	}
	...
	@Override
	public EmployeeDto addEmpCertifications(int empId, CertificationList certificationList) throws RecordNotFoundException {
		logger.info(" inside addEmpskill");
		logger.info("Employee id :" + empId + " list of skills :" + certificationList.getCertificationList());
		List<CertificationDto> empCertificationDtoList = new ArrayList<CertificationDto>();
		for (Certification certification : certificationList.getCertificationList()) {

			empCertificationDtoList.add(
					new CertificationDto(certification.getCertificationTitle(), certification.getCertificationType()));
		}

		logger.info("Get an employee object of pathvariable id to inject into empskill object");
		EmployeeDto employee = employeeReo.findById(empId);
		if (employee != null) {
			for (CertificationDto CertificationDto : empCertificationDtoList) {
				CertificationDto.setEmployee(employee);
				logger.info("Employee-Skill mapping to be added :" + CertificationDto.toString());
				certificationRepo.save(CertificationDto);
			}
		} else {
			throw new RecordNotFoundException("Record not found with employee _id : " + empId);
